...
export async function generateExercise(chapter, difficulty, type) {
    ...
    if (!client) {
        return generatedExerciseSchema.parse(buildFallbackExercise(chapter, difficulty, type));
    }
...
`;
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            ...
            return await parseExerciseResponse(rawText);
        } catch (error) {
...
export async function generateChapterSummary(chapter, subject) {
    const cacheKey = `summary:${chapter.id}`;
...
export async function correctStudentWork(imageBase64, mediaType, subject) {
    ...
    if (!client) {
        throw new Error("Service IA non configuré. Ajoutez ANTHROPIC_API_KEY pour utiliser la correction photo.");
    }
...
export async function generateSimulationSubject(subject, serie, year) {
    ...
    if (!client) {
        throw new Error("Service IA non configuré. Ajoutez ANTHROPIC_API_KEY pour générer des simulations.");
    }
