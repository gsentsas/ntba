...
import { BookOpen, ChevronDown, ChevronUp, Download, FileText } from 'lucide-react';
...
export default function Annales() {
    ...
    useEffect(() => { setPage('annales'); }, [setPage]);
    ...
    const availableYears = [...new Set(annales.map((a) => a.year))].sort((a, b) => b - a);

    return (
        <Layout>
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Annales du BAC</h1>
                    <p className="mt-1 text-sm text-slate-500">Sujets officiels avec corrigés — Session 2024</p>
                </div>
                ...
                {/* Résultats */}
                {isLoading ? (
                    <div className="flex justify-center py-16"><Spinner className="h-8 w-8 text-green" /></div>
                ) : annales.length === 0 ? (
                    <Card>
                        <CardContent className="flex flex-col items-center gap-3 py-16 text-center">
                            <BookOpen className="h-12 w-12 text-slate-200" />
                            <p className="text-slate-500">Aucune annale trouvée avec ces filtres.</p>
                        </CardContent>
                    </Card>
                ) : (
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-slate-500">Session {year}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <Badge className="border-0 bg-amber-light text-amber-700">
                                                        {exercises.length} sujet{exercises.length > 1 ? 's' : ''}
                                                    </Badge>
                                    ...
                                    {isOpen && (
                                        <CardContent className="space-y-3 pt-0">
                                            {exercises.map((ex: Exercise) => (
                                                <div key={ex.id} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                                                    <div className="flex items-start justify-between gap-3">
                                                        <div className="min-w-0 flex-1">
                                                            <p className="text-sm font-semibold text-slate-800">
                                                                {ex.annale_session ?? 'Session'}
                                                            </p>
