...
export default function Planning() {
    ...
    const generateMutation = useMutation({
        ...
        onSuccess: (data) => {
            toast.success(
                `Planning généré ! ${data.tasks_created} tâches créées, ${data.days_left} jours jusqu'au BAC.`,
            );
    ...
    const completeMutation = useMutation({
        ...
        onSuccess: (data) => {
            if (data.all_done_today) {
                toast.success(
                    '🎉 Toutes les tâches du jour terminées ! +20 pts bonus',
                );
            } else {
                        ...
                        <CardTitle className="flex items-center justify-between">
                            ...
                            {today?.stats && (
                                <Badge className="border-0 bg-green-light text-green-dark">
                                    {today.stats.completed}/{today.stats.total}{' '}
                                    tâches
                                </Badge>
                            )}
                        </CardTitle>
                            ...
                            <div className="space-y-2">
                                ...
                                {today.stats && (
                                    <p className="pt-2 text-right text-xs text-slate-400">
                                        ~{today.stats.minutes_remaining} min
                                        restantes
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="grid gap-3 sm:grid-cols-7">
                                {weekDays.map((day) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <p
                                                className={`mb-2 text-xs font-semibold uppercase ${isToday ? 'text-green' : 'text-slate-400'}`}
                                            >
                                                {format(day, 'EEE d', {
                                                    locale: fr,
                                                })}
                                                ...
                                                <div className="space-y-1">
                                                    {tasks
                                                        ...
                                                        .map((t) => (
                                                            <div
                                                                key={t.id}
                                                                className={`rounded px-1.5 py-1 text-[10px] font-medium ${t.is_completed ? 'text-slate-300 line-through' : 'text-slate-600'}`}
                                                            >
                                                                {t.title}
                                                            </div>
                                                        ))}
